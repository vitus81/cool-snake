...
extern Texture question_texture;
...
Food::Food(std::vector<Vector2> forbidden) {
  ...
  } else if (decision_var <= game_globals.prob_bonus + game_globals.prob_surprise) {
    m_category = FOOD_SURPRISE;
  } else {
  ...
  } while (vector2_in_vector(m_position,
                             forbidden)); // check for forbidden positions
  ...
  } else if (m_category == FOOD_SURPRISE) {
    m_texture = question_texture;
